...
Object.defineProperty(globalThis, 'Database', {
    get: function () {
        return {
             ...
             * @example
             * set('auction', 32) // set to world
             * set('auction', 23, player) // set to player
             */
           ...
           */
            ...
            */
             ...
             * @returns {[{key: string, value: string | number | boolean}]} An array of objects, each containing a dynamic property ID as a key and its corresponding value.
             */
            entries(player) {
                return (player || world).getDynamicPropertyIds().map((value) => ({ key: value, value: (player || world).getDynamicProperty(value) }))
            }
        };
    }
});
